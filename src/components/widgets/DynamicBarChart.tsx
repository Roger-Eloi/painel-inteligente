...
export const DynamicBarChart = ({
  ...
  // Sort data if needed
  const sortedData = config?.sort ? [...data].sort((a, b) => {
  ...
  const enhancedTitle = shouldFormatDateInTitle(categoryName) ? getDateRangeDescription(sortedData, titleText) : titleText;
  ...
  return <Card id={`widget-${widget.id}`}>
      <CardHeader>
        <CardTitle>{enhancedTitle}</CardTitle>
        {widget.description && <CardDescription className="text-xs line-clamp-2">
            {widget.description}
          </CardDescription>}
      </CardHeader>
      <CardContent>
        <ResponsiveContainer width="100%" height={300}>
          <BarChart data={sortedData}>
            ...
            <XAxis dataKey={xField} className="text-xs" label={{
            ...
            angle: -90,
            position: 'insideLeft'
          }} domain={[minDomain, maxDomain]} tickFormatter={value => value.toLocaleString('pt-BR')} />
